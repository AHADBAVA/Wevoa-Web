...
void DevServer::handleRequest(const HttpRequest& request, const HttpResponse& response) {
    if (response.statusCode == 404) {
        logger_.error("Route not found: " + request.path);
        return;
    }

    if (response.statusCode >= 500) {
        logger_.error("Request failed: " + request.path + " -> " + std::to_string(response.statusCode));
        return;
    }

    logger_.info("Request: " + request.path + " -> " + std::to_string(response.statusCode));
}
...
std::unique_ptr<WebApplication> DevServer::loadApplication() const {
