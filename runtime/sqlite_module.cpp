...
Value makeDatabaseObject(const std::shared_ptr<SQLiteConnection>& connection) {
    Value::Object object;
    object.insert_or_assign("path", Value(connection->path().generic_string()));
    object.insert_or_assign(
        "exec",
        Value(std::make_shared<NativeFunction>(
            "db.exec",
            std::nullopt,
            [connection](Interpreter&, const std::vector<Value>& arguments, const SourceSpan& span) -> Value {
                ...
                return executeExec(connection, arguments[0].asString(), parameters, span);
            })));
    object.insert_or_assign(
        "query",
        Value(std::make_shared<NativeFunction>(
            ...
            [connection](Interpreter&, const std::vector<Value>& arguments, const SourceSpan& span) -> Value {
                ...
                const Value parameters = arguments.size() == 2 ? arguments[1] : Value {};
                return executeQuery(connection, arguments[0].asString(), parameters, span);
            })));
    object.insert_or_assign(
        "scalar",
        Value(std::make_shared<NativeFunction>(
            ...
            [connection](Interpreter&, const std::vector<Value>& arguments, const SourceSpan& span) -> Value {
                ...
                const Value parameters = arguments.size() == 2 ? arguments[1] : Value {};
                return executeScalar(connection, arguments[0].asString(), parameters, span);
            })));
    ...
    sqliteModule.insert_or_assign(
        ...
        Value(std::make_shared<NativeFunction>(
            ...
            [](Interpreter&, const std::vector<Value>& arguments, const SourceSpan& span) -> Value {
                ...
                try {
                    return makeDatabaseObject(std::make_shared<SQLiteConnection>(arguments[0].asString()));
